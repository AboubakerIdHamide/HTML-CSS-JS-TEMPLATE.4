...
const observer = new IntersectionObserver(
  (links) => {
    // action when the elemnt observed
    links.forEach((link) => {
      document
        .querySelector(`header .container ul li a[href="#${link.target.id}"]`)
        .classList.toggle("link-observed", link.isIntersecting);
    });
  },
  {
    threshold: 0.5, // mean's the element should be All Showed The Range Of This Value [0-1]
  }
);

let mySections = ["home", "services", "work", "about", "skills", "contact"];
mySections.forEach((sectionId) => {
...
let socialI = document.querySelector(
    ".video .container .ctrl-label .share-label i:first-of-type"
  ),
  labelIcons = document.querySelector(
